...
function getCardOne() {
  for (i = 0; i < 1; i++) {
    ...
    $img.src = `../img/blackjack/${newRandomCard.src}`;
...
function startTabel() {
  for (i = 0; i <= 3; i++) {
    ...
    if (i % 2 === 0) {
      ...
      //첫장은 뒷면
      if (i === 0) {
        $img.src = "../img/blackjack/card_back.png";
      }
      $dealerCard.appendChild($img);
...
function compareTotalSum() {
  $dealerCard.firstElementChild.src = `../img/blackjack/${newCardArr[0].src}`; // 뒤집어진 카드 다시 이미지 설정
  if (
